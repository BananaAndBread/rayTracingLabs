...
public:
    ...
    Vertex(float3 position, float3 normal) : position(position), tex(float3{0.0, 0.0, 0.0}), color(float3{1.0, 1.0, 1.0})
    {
        this->normal = normalize(normal);
    };
    Vertex(float3 position, float3 normal, float3 tex) : position(position), tex(tex), color(float3{1.0, 1.0, 1.0})
    {
        this->normal = normalize(normal);
    };
    Vertex(float3 position, float3 normal, float3 tex, float3 color) : position(position), tex(tex), color(color)
    {
        this->normal = normalize(normal);
    };
