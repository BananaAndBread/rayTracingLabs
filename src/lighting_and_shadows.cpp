...
int LightingAndShadows::LoadGeometry(std::string filename)
{
    ...
    bool ret = tinyobj::LoadObj(&attrib, &shapes, &materials, &warn, &err, filename.c_str(), filepath.parent_path().string().c_str(), true);
    ...
    for (size_t s = 0; s < shapes.size(); s++)
    {
        ...
        for (size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++)
        {
            ...
            triangle->SetEmissive(float3(mat.emission));
...
Payload LightingAndShadows::Hit(const Ray &ray, const IntersectableData &data, const MaterialTriangle *triangle, const unsigned int max_raytrace_depth) const
{
    ...
    for (auto &light : lights)
    {
        ...
        {
            payload.color += light->color * triangle->diffuse_color * std::max(.0f, dot(to_light_ray.direction, n));
            Ray from_light_ray(light->position, x - light->position);
            float3 specular_direction = from_light_ray.direction - 2.f * dot(from_light_ray.direction, n) * n;
            payload.color += light->color * triangle->specular_color * powf(std::max(dot(ray.direction, specular_direction), 0.f), triangle->specular_exponent);
        }
    }
